...
import { 
    Play, Clock, Globe, ChevronRight, ChevronLeft, BookOpen, Save, Plus, Trash2 
} from 'lucide-react';
import useEmblaCarousel from 'embla-carousel-react';
import Autoplay from 'embla-carousel-autoplay';

export default function PlayV3() {

    const [activeTab, setActiveTab] = useState('about');
    
    // Mock Data
    const archives = [
        { id: 1, title: 'Chapter 4: The Hidden Valley', date: '2 hours ago', status: 'In Progress', playTime: '12h 30m', location: 'Elven Ruins' },
            ...
            <main className="pt-32 pb-20 max-w-7xl mx-auto px-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Recent Activity / Archives (New Style from Component Library) */}
                        <div className="w-full bg-white/50 rounded-2xl p-4 border border-slate-100 shadow-[0_2px_2px_#CAEBFF] sticky top-32 backdrop-blur-sm">
                            {/* Header */}
                            ...
                            <div className="h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-indigo-200 scrollbar-track-transparent space-y-3">
                                {archives.map((save) => (
                                    <div key={save.id} className="group flex items-center justify-between p-3 bg-white rounded-xl border border-slate-50 hover:shadow-[0_2px_2px_#CAEBFF] transition-all hover:-translate-y-0.5 cursor-pointer">
                                        <div className="flex items-center gap-3">
                                            <div className="relative w-10 h-10 bg-indigo-50 group-hover:bg-[#818CF8] rounded-lg flex items-center justify-center text-[#818CF8] group-hover:text-white transition-all duration-300">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-bold text-slate-700 group-hover:text-[#818CF8] transition-colors">{save.title}</h4>
                                                <span className="text-[10px] text-slate-400 font-medium font-mono">{save.date}</span>
                                            </div>
                                        </div>
                                        <button 
